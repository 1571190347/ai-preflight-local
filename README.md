...
**v0.3.0 · 本地运行 · 中文界面 · MIT 开源**　[English](README.en.md)

在自己的电脑上检查公网 IP、分流出口、Claude / ChatGPT 连接、IP 画像、DNS、WebRTC 和浏览器环境。界面和后端都运行在本机；外部检测默认关闭，启用后仍需点击对应按钮才会发送请求。

工具展示来源明确的网络观察，帮助定位“浏览器能访问，后台为什么失败”等问题。它无法读取平台内部信任分、账号状态或封号概率，也不把缺失数据当作通过。
...
| `PREFLIGHT_VERSION` | 指定稳定版，例如 `v0.3.0`；默认读取最新 Release |
| `PREFLIGHT_NO_START=1` | 安装后暂不启动 |
| `PREFLIGHT_NO_OPEN=1` | 启动时不自动打开浏览器 |

这些是运行安装器或启动器之前设置的环境变量。例如 macOS / Linux 可把 `PREFLIGHT_VERSION=v0.3.0` 放在管道末端的 `sh` 前；Windows 可先执行 `$env:PREFLIGHT_VERSION = 'v0.3.0'`。

### 手动运行发行包

从 [Releases](https://github.com/1571190347/ai-preflight-local/releases) 下载 `ai-preflight-local-v0.3.0.zip` 或 `.tar.gz`，对照同一版本的 `SHA256SUMS` 校验后解压。发行附件包含构建后的 `dist/`，手动方式需自行准备 Node.js 22.13+。在解压后的项目目录运行：
...
| 模块 | 提供的操作与结果 |
| --- | --- |
| 总览 / 后台连接 | 本地浏览器基础检查；主动检查 Cloudflare HTTPS、ipwho.is、IANA RDAP 三个后台目的地，逐项显示 DNS 来源、HTTP 结果、耗时或失败原因 |
| IP 与分流 | 自选出口源，对比浏览器与本地服务的 IPv4 / IPv6、HTTP 出口、国家和 trace 字段；默认遮罩 IP |
...
1. 在总览运行本地基础检查，确认本地页面与 API 正常。
2. 阅读目的地说明，启用外部检测，点击“检查后台连接”，区分浏览器路径和 Node 后台路径的问题。
3. 在“IP 与分流”选择来源，分别观察浏览器和后台出口，再进入 Claude 或 ChatGPT 页面运行对应检查。
4. 按需查询 IP 画像、DNS、WebRTC 或 Ping。出现“无法确认”时保留具体原因，不把它转换成账号风险结论。
